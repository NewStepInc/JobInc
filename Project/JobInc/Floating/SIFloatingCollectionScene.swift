...
class SIFloatingCollectionScene: SKScene {
    ...
    override func touchesEnded(touches: Set<UITouch>, withEvent event: UIEvent?) {
        if mode != .Moving, let touch = touchPoint {
            if let node = nodeAtPoint(touch) as? SIFloatingNode {
                ...
                GlobalUtil.jobCount -= 1
                if GlobalUtil.jobCount > 0 {
                    transitJobInc()
                }else{
                    transitFinal()
                }
    ...
    func transitJobInc()
    {
        ...
        let vc: JobIncViewController = storyboard.instantiateViewControllerWithIdentifier("boardJobInc") as! JobIncViewController
    ...
    {
        let storyboard: UIStoryboard = UIStoryboard(name: "Main", bundle: nil)
        let vc: FinalViewController = storyboard.instantiateViewControllerWithIdentifier("boardFinal") as! FinalViewController
        viewController!.presentViewController(vc, animated: true, completion: nil)
    }
